...
const slides = [
  {
    ...
    description: "Discover premium fashion, electronics & accessories curated just for you.",
    cta: "Shop Now",
    link: "/products",
  ...
  },
  {
    ...
    description: "Top-rated headphones, smart watches & accessories at the best prices.",
  ...
  },
  {
    ...
    description: "Beautiful home décor, candles, and organizers to elevate your lifestyle.",
    cta: "Shop Home",
...
export const HeroBanner = () => {
  ...
  return (
    <section className="relative w-full aspect-[16/9] sm:aspect-[21/9] md:aspect-[3/1] overflow-hidden bg-secondary">
      ...
      <button
        onClick={prev}
        className="absolute left-2 sm:left-4 top-1/2 -translate-y-1/2 p-2 sm:p-2.5 rounded-full bg-white/20 backdrop-blur-sm text-white hover:bg-white/30 transition-colors touch-manipulation"
      ...
      <button
        onClick={next}
        className="absolute right-2 sm:right-4 top-1/2 -translate-y-1/2 p-2 sm:p-2.5 rounded-full bg-white/20 backdrop-blur-sm text-white hover:bg-white/30 transition-colors touch-manipulation"
        ...
        {slides.map((_, i) => (
          <button
            ...
            className={`h-1.5 rounded-full transition-all touch-manipulation ${
              i === current ? "w-6 bg-primary" : "w-1.5 bg-white/50"
            }`}

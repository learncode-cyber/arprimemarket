...
const defaultSlides: Slide[] = [
  { title: "New Season Collection", subtitle: "Up to 40% Off", description: "Discover our carefully curated collection of premium products at unbeatable prices.", cta_text: "Shop Now", cta_link: "/products", image: "https://images.unsplash.com/photo-1441986300917-64674bd600d8?w=1200&q=80&auto=format&fit=crop", badge: "🔥 Hot Deal", accent: "from-rose-900/80 via-rose-900/40" },
  { title: "Premium Electronics", subtitle: "Latest Gadgets", description: "Shop the latest electronics, gadgets, and accessories from top brands.", cta_text: "Explore Electronics", cta_link: "/products?category=Electronics", image: "https://images.unsplash.com/photo-1468495244123-6c6c332eeece?w=1200&q=80&auto=format&fit=crop", badge: "⚡ New Arrivals", accent: "from-slate-900/80 via-slate-900/40" },
  { title: "Home & Living", subtitle: "Transform Your Space", description: "Elevate your living space with our premium home decor collection.", cta_text: "Shop Home", cta_link: "/products?category=Home", image: "https://images.unsplash.com/photo-1616046229478-9901c5536a45?w=1200&q=80&auto=format&fit=crop", badge: "✨ Trending", accent: "from-emerald-900/70 via-emerald-900/30" },
];

export const HeroBanner = () => {
  const cmsData = useSectionContent<{ slides: Slide[] }>("hero_banner");
  const slides = cmsData?.slides?.length ? cmsData.slides : defaultSlides;
    ...
    <section className="relative w-full aspect-[16/9] sm:aspect-[21/9] md:aspect-[2.8/1] overflow-hidden bg-secondary">
      <AnimatePresence mode="wait">
        ...
        >
          ...
          <img src={slide.image} alt={slide.title} className="w-full h-full object-cover relative z-[1]" loading={current === 0 ? "eager" : "lazy"} />
      ...
      <div className="absolute inset-0 z-[3] flex items-end sm:items-center">
        <div className="max-w-7xl mx-auto w-full px-4 sm:px-6 pb-10 sm:pb-0">
          <AnimatePresence mode="wait">
            <motion.div key={current} initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }} transition={{ duration: 0.45, delay: 0.15 }} className="max-w-lg">
              ...
              <span className="block mb-2">
                <span className="inline-block px-2.5 py-1 rounded-md bg-primary text-primary-foreground text-[10px] sm:text-xs font-semibold uppercase tracking-wider">{slide.subtitle}</span>
              </span>
              <h1 className="font-display text-2xl sm:text-4xl md:text-5xl lg:text-6xl font-extrabold text-white leading-[1.1] mb-2 sm:mb-3 drop-shadow-lg">{slide.title}</h1>
              <p className="text-xs sm:text-sm md:text-base text-white/85 mb-5 sm:mb-6 leading-relaxed max-w-md drop-shadow">{slide.description}</p>

      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="fixed sm:absolute inset-x-4 sm:inset-x-auto top-1/2 sm:top-full -translate-y-1/2 sm:translate-y-0 sm:right-0 sm:mt-1.5 sm:w-64 max-h-[70vh] sm:max-h-none bg-popover border border-border rounded-xl shadow-2xl z-[100] overflow-hidden"
          >
            {/* Search bar */}
            ...
            <div className="max-h-[50vh] sm:max-h-64 overflow-y-auto scrollbar-hide py-1">
              {filtered.length === 0 ? (
                <p className="text-sm sm:text-xs text-muted-foreground text-center py-4">No currency found</p>
              ) : (
                filtered.map((c) => (
                  <button
                    key={c.code}
                  ...
                  >
                    <span className="font-bold w-10 sm:w-8 text-left">{c.code}</span>

  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6 sm:py-10">
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
        <h1 className="font-display text-2xl sm:text-3xl font-bold text-foreground mb-1">{t("cart")}</h1>
        <p className="text-xs sm:text-sm text-muted-foreground mb-6 sm:mb-8">{totalItems} {t("itemsInCart")}</p>
      </motion.div>
      ...
      {items.length === 0 ? (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-16 bg-card border border-border rounded-2xl">
          ...
          <p className="text-sm text-muted-foreground mb-5">{t("emptyCart")}</p>
